...
- **`epos ready` does not filter claimed tickets**: `ReadyFilter` reads only the ticket file's `status` field, not the sidecar. So `ready` may list a ticket another agent currently holds. Always *attempt* `epos claim` and treat failure as "skip, try the next one."
