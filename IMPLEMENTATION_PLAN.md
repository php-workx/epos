...
| E3-T8 | Implement dependency graph (pure only) | `ReadyFilter`, `BlockedFilter`, `DetectCycles`, `FilterChildren`, `FilterReadyChildren` in `ticket/graph/` — pure functions only, no I/O | All graph tests pass |
| E3-T9 | Implement child discovery (I/O-bound) | `ListAllChildren`, `ListReadyChildren`, `HasChildren` in `ticket/store/store.go` — moved from graph/ since they require filesystem I/O | Parent-child hierarchy works |
...
| Inline claim fields in fabrikk frontmatter | Migration confusion | Phase E5-T6 explicitly removes inline claim writes; sidecar reads fall back to inline fields during transition |
